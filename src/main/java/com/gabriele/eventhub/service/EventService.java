...
public class EventService {
	...
	private EventResponseDTO toDTO(Event event) {
		...
		if (event.getTags() != null) {
			dto.setTagNames(event.getTags().stream().map(Tag::getName).collect(Collectors.toList()));
		}

		return dto;
	}

	public List<EventResponseDTO> findByFilters(LocalDateTime startDate, LocalDateTime endDate, Long venueId,
			Long organizerId, Long tagId) {
		List<Event> events = eventRepository.findByFilters(startDate, endDate, venueId, organizerId, tagId);
		List<EventResponseDTO> result = new ArrayList<>();
		for (Event event : events) {
			result.add(toDTO(event));
		}
